...
class enrol_manual_enrol_users_form extends moodleform {
    ...
    public function definition() {
        ...
        $mform->addElement('header', '', get_string('enrolmentoptions', 'enrol'));
        $roles = get_assignable_roles($context);
        $mform->addElement('select', 'assignroles', get_string('assignroles', 'enrol_manual'), $roles, ['multiple' => true]);
        $keys = array_keys($roles);
        $defaultrole = end($keys);
        $mform->setDefault('assignroles', $defaultrole);

        $mform->addElement('checkbox', 'recovergrades', get_string('recovergrades', 'enrol'));
        $mform->addElement('select', 'extendperiod', get_string('defaultperiod', 'enrol_manual'), $periodmenu);
        $mform->setDefault('extendperiod', $defaultperiod);
        $mform->addElement('select', 'extendbase', get_string('startingfrom'), $basemenu);
        $mform->setDefault('extendbase', $extendbase);

        $mform->addElement('header', 'selection', get_string('selection', 'enrol_manual'));
