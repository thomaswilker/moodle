...
class setting_dependency_disabledif_equals extends setting_dependency {
    ...
    public function is_locked() {
        ...
        if ($this->setting->get_status() !== base_setting::NOT_LOCKED || $this->evaluate_condition($this->setting->get_value())) {
            return true;
        }
    ...
    protected function process_value_change($oldvalue) {
        ...
        if ($this->evaluate_condition($this->setting->get_value())) {
    ...
    protected function process_status_change($oldstatus) {
        ...
        if ($currentstatus == base_setting::NOT_LOCKED) {
            if ($prevalue == base_setting::LOCKED_BY_HIERARCHY && !$this->evaluate_condition($this->setting->get_value())) {
     ...
     * @return bool
     */
    public function evaluate_condition($value) {
