...
$messageformcontroller = new \block_message_broadcast\form_controller();

$PAGE->set_title(get_string('newmessage', 'block_message_broadcast'));
...
if ($mform->is_cancelled()) {
    ...
    if (!$messageformcontroller->admin) {
        // Force coruse edit for non admin.
        $data->courseids = array($messageformcontroller->courseid);
    }
    $managemessages = new \block_message_broadcast\manage();
    $managemessages->save_message($data);
    redirect($messageformcontroller->get_managemessageurl());
}
