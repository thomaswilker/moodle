...
class converter implements \core_files\converter_interface {
    ...
    public function start_document_conversion(\core_files\conversion $conversion) {
        ...
        if (empty($issuerid)) {
            throw new moodle_exception('The OAuth 2 service is not configured');
        }

        $issuer = \core\oauth2\api::get_issuer($issuerid);
        if (empty($issuer)) {
            throw new moodle_exception('The OAuth 2 service is invalid');
        }
        ...
        if (strpos($originalname, '.') === false) {
            throw new moodle_exception('The filename has no extension');
        }
        ...
        if (empty($uploadurl)) {
            throw new moodle_exception('Could not upload document to Google');
        }
