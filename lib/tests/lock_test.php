...
class lock_testcase extends advanced_testcase {
    ...
    public function test_locks() {
        ...
        $dblockfactory = new \core\lock\db_lock_factory('test', array());
        $this->run_on_lock_factory($dblockfactory);

        $filelockfactory = new \core\lock\file_lock_factory('test', array());
        $this->run_on_lock_factory($filelockfactory);

        $upgradelockfactory = new \core\lock\upgrade_lock_factory('test', array());
        $this->run_on_lock_factory($upgradelockfactory);

        // The memcache factories are not testable without a working memcache server,
        // to test them would be identical to the above examples, except you would
        // have to specify a valid memcache server in the configuration.
    }
}
