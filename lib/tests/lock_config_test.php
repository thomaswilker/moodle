...
class lock_config_testcase extends advanced_testcase {
    ...
    public function test_lock_config() {
        ...
        if (isset($CFG->lock_config)) {
            $original = $CFG->lock_config;
        }

        // Test no configuration.
        unset($CFG->lock_config);
        ...
        $CFG->lock_config = array('default'=>array('factory'=>'\core\lock\file_lock_factory'));

        $factory = \core\lock\lock_config::get_lock_factory('cache');
        $this->assertTrue($factory instanceof \core\lock\file_lock_factory,
                          'Get a default factory with only a default configuration');

        $CFG->lock_config = array('default'=>array('factory'=>'\core\lock\file_lock_factory'),
                                  'cache'=>array('factory'=>'\core\lock\db_lock_factory'));

        $factory = \core\lock\lock_config::get_lock_factory('cache');
        $this->assertTrue($factory instanceof \core\lock\db_lock_factory,
                          'Get a default factory with a specific configuration');

        if ($original) {
            $CFG->lock_config = $original;
        } else {
            unset($CFG->lock_config);
        }
    }
