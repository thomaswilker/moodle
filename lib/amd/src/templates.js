...
define(['core/mustache',
    ...
    /** @var {string[]} iconMap - Cache of icon mapping. */
    var iconMap = false;
    ...
    Renderer.prototype.pixHelper = function(context, sectionText, helper) {
        ...
        // We are guaranteed to have an iconmap here.
        if (typeof iconMap[component][key] != 'undefined') {
            key = iconMap[component][key];
        }

        var url = coreurl.imageUrl(key, component);

        var fontawesome = key.indexOf('fa-') === 0;
        var pixlist = [];
        if (fontawesome) {
            pixlist = key.split('+');
        }
    ...
    Renderer.prototype.render = function(templateName, context, themeName) {
        ...
        var loadIconMap = this.getIconMap(themeName);

        loadIconMap.then(function() {
            return this.getTemplate(templateName, true);
     ...
     * @return {Promise} JQuery promise resolved with the array of icons when the icon map is loaded.
     */
    Renderer.prototype.getIconMap = function(themeName) {
        var deferred = $.Deferred();

        if (iconMap !== false) {
            deferred.resolve(iconMap);
        } else {
            // Check local storage.
            // Now try local storage.
            var cached = storage.get('core_output/icon_map-' + themeName);
            ...
            if (cached) {
                ...
                // Need to fetch it.
                ajax.call([{
                    methodname: 'core_output_load_iconmap',
                    args: {},
                    done: function(map) {
                        var fetchMap = [];
                        $.each(map, function(index, value) {
                            if (typeof fetchMap[value.component] == 'undefined') {
                                fetchMap[value.component] = [];
                            }
                            fetchMap[value.component][value.pix] = value.to;
                        }.bind(this));
                        storage.put('core_output/icon_map-' + themeName, map);
                        iconMap = map;
