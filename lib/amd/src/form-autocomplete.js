...
define(['jquery', 'core/log', 'core/str', 'core/templates', 'core/notification'], function($, log, str, templates, notification) {
     ...
     * @param {Boolean} casesensitive Do a case sensitive search.
     */
    var updateSuggestions = function(query, inputId, suggestionsId, originalSelect, multiple, tags, casesensitive) {
        ...
        templates.render(
            ...
            suggestionsElement.children().each(function(index, node) {
                node = $(node);
                var texttosearch = node.text();
                var querytext = query;
                if (!casesensitive) {
                    texttosearch = texttosearch.toLowerCase();
                    querytext = querytext.toLowerCase();
                }
                if (texttosearch.indexOf(querytext) > -1) {
                ...
                case KEYS.DOWN:
                    ...
                    if (inputElement.attr('aria-expanded') === "true") {
                        ...
                        if (!inputElement.val() && ajax) {
                            ...
                            updateSuggestions(searchtext, inputId, suggestionsId, originalSelect, multiple, tags, casesensitive);
                        }
                    }
