...
define(['jquery', 'core/log', 'core/str', 'core/templates', 'core/notification'], function($, log, str, templates, notification) {
    ...
    var activateItem = function(index, inputId, datalistId) {
        ...
        newDatalist.children().attr('data-active', false);
        element.attr('data-active', true).attr('id', itemId);
        newInput.attr('aria-activedescendant', itemId);
    };

    var activateNextItem = function(inputId, datalistId) {
        var newDatalist = $(document.getElementById(datalistId));
        var element = newDatalist.children('[data-active=true]');
    ...
    var selectCurrentItem = function(inputId, datalistId, multiple) {
        var newInput = $(document.getElementById(inputId));
        var newDatalist = $(document.getElementById(datalistId));
        var query = newDatalist.children('[data-active=true]').html();
        // Here loop through datalist and set val to join of all selected items.
        newDataList.children('[data-active=true]').attr('aria-selected', true);
        var allText = '';
        newDataList.children('[aria-selected=true]').each(function(index, ele) {
            allText += ', ' + ele.html();
        });
        newInput.val(query);
        if (!multiple) {
    ...
    var addNavigation = function(inputId, datalistId, downArrowId, originalSelect, multiple) {
        ...
        inputElement.on('keydown', function(e) {
            switch (e.keyCode) {
                ...
                case KEYS.UP:
                    ...
                    return false;
                case KEYS.ENTER:
                    selectCurrentItem(inputId, datalistId, multiple);
                    return false;
                case KEYS.ESCAPE:
                    if (inputElement.attr('aria-expanded') === "true") {
                        closeSuggestions(inputId, datalistId);
    ...
    return /** @alias module:core/form-autocomplete */ {
        ...
        enhance: function(selector, tags, ajax, placeholder) {
            ...
            $.when(renderInput, renderDatalist).done(function(input, datalist) {
                originalSelect.after(datalist);
                originalSelect.after(input);
                originalLabel.attr('for', inputId);
                addNavigation(inputId, datalistId, downArrowId, originalSelect, multiple);
