...
foreach ($jsfiles as $modulename => $jsfile) {
    ...
    // I don't care about C compatibility, these args are the wrong way round!
    if (strpos($js, $search) === false) {
        // We can't call debugging because we only have minimal config loaded.
        error_log('JS file: ' . $shortfilename . ' does not contain a javascript module in AMD format. "define()" not found.');
    }
