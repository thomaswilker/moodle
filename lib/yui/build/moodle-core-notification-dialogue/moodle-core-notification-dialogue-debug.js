...
Y.extend(DIALOGUE, Y.Panel, {
    ...
    visibilityChanged : function(e) {
        ...
        if (e.attrName === 'visible') {
            ...
            if (!e.prevVal && e.newVal) {
                ...
                Y.one('#'+this.get('id')).on('keydown', function(e) {
                    if (e.keyCode === 9) { // Tab key
                        var target = Y.YUI2.util.Event.getTarget(e);
                        this.trapFocus(this, target.get('id'));
                    }
                }, this);
            }
     ...
     */
    focusableItems : function(dialogid, nodes) {
        if (nodes !== '' || nodes !== 'undefined') {
            for(var i = 0; i < nodes.length; i++) {
                if (nodes[i] !== '' && Y.one('#'+nodes[i]) !== null) {
                    var ids = Y.one('#'+nodes[i]).get('children').getAttribute('id');
                    if (Y.one('#'+nodes[i]).getAttribute('type') !== 'hidden') {
                        FOCUSABLEITEMS[dialogid].push(nodes[i]);
                    }
                    this.focusableItems(dialogid, ids);
                }
            }
        }
    }
