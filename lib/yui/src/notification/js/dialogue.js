...
Y.extend(DIALOGUE, Y.Panel, {
        ...
        if (e.attrName === 'visible') {
            ...
            if (!e.prevVal && e.newVal) {
                ...
                //Y.use("node", function(Y) {
                    //Y.all('#'+this.get('id')).each(function(node) {
                        Y.one('#'+this.get('id')).on('keydown', function(e) {// alert(node.toSource());
                            if (e.keyCode === 9) { // Tab key
                                //alert(Y.one(document.activeElement).get('id'));
                                //alert(e.currentTarget);
                                this.trapFocus(this, Y.one(document.activeElement).get('id'));
                            }
                        }, this);
                    //});
                //});
    ...
    trapFocus : function(node, currentTarget) {
        var bb = this.get('boundingBox'),
            content;

        //var focusItemsArray = [];//new Array();
        ...
        //alert(node.get('id'));//.toSource());
        this.getFocusItems(dialogid, dialogcontentid);
        //alert(FOCUSITEMS[dialogid].toSource());

        var lastitem = FOCUSITEMS[dialogid][FOCUSITEMS[dialogid].length - 1];//alert(lastitem + '  ' + currentTarget);
        if (lastitem === currentTarget){
            Y.one('#'+FOCUSITEMS[dialogid][0]).focus();
        }
        //for (var i = 0; i < focusItemsArray.length; i++) {
            //focusItemsArray.push(bbroot.get('id'));
        //}
        //var contentdialog = Y.one('#'+bbroot);//.next();
        ...
        //alert('boodx');
        //alert(evt.target);
        //alert(this.getTargets());
        /*
         Y.all(EXISTING_WINDOW_SELECTOR).each(function (node) {
         //var focusItemsArray = Y.one('#' + this.get('id')).getFocus(node); //getFocusItems(node); //dialogContainerNode);
         //alert(node.toSource());
         dialog.firstFocusItem = focusItemsArray[0];
         dialog.lastFocusItem = focusItemsArray[focusItemsArray.length-1];

         });
         //var focusItemsArray = Y.one('#' + this.get('id')).getFocusItems(node); //dialogContainerNode);
         //dialog.firstFocusItem = focusItemsArray[0];
         //dialog.lastFocusItem = focusItemsArray[focusItemsArray.length-1];
         //}
     ...
     * @returns {Array}
     */
    getFocusItems : function(dialogid, nodes) {
        //var items = [];//new Array();
        if (nodes !== '' || nodes !== 'undefined') {
            for(var i = 0; i < nodes.length; i++) {
                if ((nodes[i] !== 'undefined' || nodes[i] !== '' || nodes[i] !== null) && Y.one('#'+nodes[i]) !== null) {
                    var ids = Y.one('#'+nodes[i]).get('children').getAttribute('id');
                    this.getFocusItems(dialogid, ids);
                    //items.push(nodes[i]);//
                    FOCUSITEMS[dialogid].push(nodes[i]);
                }
            }
        }
        //return items;
         ...
         if (node.hasChildNodes()) {
            ...
            for (var i = 0; i < node.children.length; i++) {
                if (node.children[i].hasChildNodes()) {
                    getFocusItems(node.children[i], array);
