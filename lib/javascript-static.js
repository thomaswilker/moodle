...
M.util.js_pending = function(uniqid) {
    if (uniqid !== false) {
        console.log('js_pending: ' + uniqid);
        M.util.pending_js.push(uniqid);
    }

    console.log('result: ' + M.util.pending_js.length);
    return M.util.pending_js.length;
};

M.util.js_debug = function(uniqid, debug) {
    M.util.debug.push(uniqid + ':' + debug);
}
/**
...
M.util.js_complete = function(uniqid) {
    ...
    console.log('js_complete: ' + uniqid);
    if (index >= 0) {
