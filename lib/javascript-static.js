...
/**
 * Bulk loads strings via AJAX.
 *
 * The point here is, devs should still load all the strings they require for JS,
 * but they do not have to do it in php as part of the page load. This is recommended
 * for most cases - except for a rarely used function with a lot of strings. In this case
 * it could be better to load the strings once when the function is first used.
 *
 * @param {String} identifier string identifier
 * @param {String} component the component providing the string
 * @param {Object|String} a optional variable to populate placeholder with
 */
M.util.strings_for_js = function(identifiers, component, callback) {
    var missingstrings = [], i;

    // Determine strings that are not already loaded.
    if (typeof M.str[component] === "undefined") {
        missingstrings = identifiers;
    } else {
        for (i = 0; i < identifiers.length; i++) {
            if (typeof M.str[component][identifiers[i]] === "undefined") {
                missingstrings.push(identifiers[i]);
            }
        }
    }

    // If all the strings are already loaded, no need for ajax.
    if (missingstrings.length === 0) {
        return callback();
    }

    // TODO - make an ajax call to /lib/ajaxlib/strings.php to load the strings.
