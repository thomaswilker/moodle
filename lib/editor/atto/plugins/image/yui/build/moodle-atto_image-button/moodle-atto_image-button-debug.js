...
M.atto_image = M.atto_image || {
    ...
    set_image : function(e, elementid) {
        ...
        input = e.currentTarget.get('parentNode').one('#atto_image_presentation');
        var presentation = input.get('checked');

        e.preventDefault();

        if (alt === '' && !presentation) {
            var alert = e.currentTarget.get('parentNode').one('#atto_image_altwarning');
    ...
    get_form_content : function(elementid) {
        var content = Y.Node.create('<form class="atto_form">' +
                             ...
                             '</label>' +
                             '<input type="text" value="" id="atto_image_altentry" size="32"/>' +
                             '<label for="atto_image_presentation">' + M.util.get_string('presentation', 'atto_image') +
                             '</label>' +
