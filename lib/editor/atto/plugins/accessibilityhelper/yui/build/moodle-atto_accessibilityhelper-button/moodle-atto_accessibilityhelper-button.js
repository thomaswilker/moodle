...
M.atto_accessibilityhelper = M.atto_accessibilityhelper || {
    ...
    display_ui : function(e, elementid) {
        ...
        dialogue.set('bodyContent', M.atto_accessibilityhelper.get_form_content(elementid));
    ...
    init : function(params) {
        ...
        // Hide the entire accessibilty group from browsers.
        var toolbar = M.editor_atto.get_toolbar_node(params.elementid);
        var group = toolbar.one('.screenreader_group');

        if (group) {
            group.addClass('accesshide');
        }
        // Add focus/blur listeners to the group so the accessibility group is shown only
        // when navigated to with the keyboard.
        group.on('blur', function() {
            this.addClass('accesshide');
        }, group);
        group.on('focus', function() {
            this.removeClass('accesshide');
        }, group);
    },
    ...
    get_form_content : function(elementid) {
        // Current styles.
        var html = '<div><p for="atto_accessibilityhelper_styleslabel">' +
                M.util.get_string('liststyles', 'atto_accessibilityhelper') +
                '<br/>' +
                '<span id="atto_accessibilityhelper_liststyles" ' +
                'aria-labelledby="atto_accessibilityhelper_styleslabel"/></p></div>';


        var content = Y.Node.create(html);

        content.one('#atto_accessibilityhelper_liststyles').append(this.list_styles(elementid));

        // Current links.
        html = '<p for="atto_accessibilityhelper_linkslabel">' +
                M.util.get_string('listlinks', 'atto_accessibilityhelper') +
                '<br/>' +
                '<span id="atto_accessibilityhelper_listlinks" ' +
                'aria-labelledby="atto_accessibilityhelper_linkslabel"/></p>';

        content.append(html);
        content.one('#atto_accessibilityhelper_listlinks').append(this.list_links(elementid));

        // Current images.
        html = '<p for="atto_accessibilityhelper_imageslabel">' +
                M.util.get_string('listimages', 'atto_accessibilityhelper') +
                '<br/>' +
                '<span id="atto_accessibilityhelper_listimages" ' +
                'aria-labelledby="atto_accessibilityhelper_imageslabel"/></p>';

        content.append(html);
        content.one('#atto_accessibilityhelper_listimages').append(this.list_images(elementid));
        return content;
    }
