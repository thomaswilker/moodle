...
M.atto_equation = M.atto_equation || {
    ...
    init : function(params) {
        ...
        if (params.texfilteractive) {
            ...
            M.editor_atto.add_toolbar_button(params.elementid, 'equation', iconurl, params.group, this.display_chooser, this);
        }
    },
    ...
    get_library_html : function(elementid) {
        ...
        for (group = 1; group < 5; group++) {
            ...
            for (i = 0; i < examples.length; i++) {
                if (examples[i]) {
                    content += '<button data-tex="' + Y.Escape.html(examples[i]) + '">$$' + examples[i] + '$$</button>';
                }
            }
            content += '</div>';
        }
