    ...
    TEMPLATES = {
        FORM: '' +
            '<form class="atto_form">' +
                ...
                '<div class="mdl-align">' +
                    '<br/>' +
                    '<button id="{{CSS.SUBMIT}}">{{get_string "saveequation" component}}</button>' +
                '</div>' +
            '</form>',
        LIBRARY: '' +
            '<div id="{{CSS.LIBRARY}}">' +
                '<ul>' +
                    '{{#each library}}' +
                        '<li><a href="#{{../CSS.LIBRARY_GROUP_PREFIX}}{{@key}}">{{get_string groupname ../component}}</a></li>' +
                    '{{/each}}' +
                '</ul>' +
                '<div>' +
                    '{{#each library}}' +
...
Y.namespace('M.atto_equation').Button = Y.Base.create('button', Y.M.editor_atto.EditorPlugin, [], {
    ...
    _displayDialogue: function() {
        ...
        if (equation) {
            content.one('#atto_equation_equation').set('text', equation);
        }
    ...
    _getDialogueContent: function() {
        ...
        content.one(SELECTORS.SUBMIT).on('click', this._setEquation, this);
        content.one(SELECTORS.EQUATION_TEXT).on('valuechange, keyup, mouseup', this._updatePreview, this);
        content.delegate('click', this._selectLibraryItem, SELECTORS.LIBRARY_BUTTON, this);
    ...
    _getLibraryContent: function() {
        ...
        Y.Handlebars.registerHelper('split', function(delimiter, str) {
            var parts,
                current,
                out;
            if (typeof delimiter === "undefined" || typeof str === "undefined") {
                Y.log('Handlebars split helper: String and delimiter are required.', 'debug');
                return '';
            }
