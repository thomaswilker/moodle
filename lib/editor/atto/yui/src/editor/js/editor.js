...
M.editor_atto = M.editor_atto || {
    ...
    init : function(params) {
        ...
        // Fire a custom event once everything else is setup.
    },
    ...
    publish_events : function() {
        Y.Event.define("atto:selectionchanged", {
            ...
            on: function (node, sub, notifier) {
                ...
                sub.attoKeyDownDetach = node.on('keydown', Y.throttle(this.changeHandler), this, params);
                sub.attoMouseUpDetach = node.on('mouseup', Y.throttle(this.changeHandler), this, params);
                sub.attoFocusDetach = node.on('focus', Y.throttle(this.changeHandler), this, params);
            },
            ...
            delegate: function(node, sub, notifier, filter) {
                var params = { notifier: notifier, sub: sub };
                sub.attoKeyDownDetachDelegate = node.delegate('keydown', Y.throttle(this.changeHandler), filter, this, params);
                sub.attoMouseUpDetachDelegate = node.delegate('mouseup', Y.throttle(this.changeHandler), filter, this, params);
                sub.attoFocusDetachDelegate = node.delegate('focus', Y.throttle(this.changeHandler), filter, this, params);
            },
    ...
    get_selected_nodes : function(elementid) {
         ...
         * @param {Integer} skipChildren - When returning from a child to a parent, this is now many children to skip.
         * @param {Y.NodeList} leafNodes - The list of leaf nodes found.
         * @return false (Eventually always)
         */
        var findLeafNodes = function(node, endNode, skipChildren, leafNodes) {
            // if there are child nodes and we didn't come from a child node
            if (endNode === node) {
                if (endNode === node && startNode === node && !skippedonce && node.hasChildNodes()) {
                    skippedonce = true;
                } else {
            ...
            if (node.hasChildNodes()) {
                ...
                if (child) {
                    return findLeafNodes(child, endNode, 0, leafNodes);
                }
            } else {
            ...
            return findLeafNodes(parent, endNode, currentindex + 1, leafNodes);
        };

        findLeafNodes(startNode, endNode, 0, leafNodes);
        return leafNodes;
    },
