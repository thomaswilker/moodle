...
M.editor_tinymce.init_editor = function(Y, editorid, options) {
    M.util.js_pending('editor:' + editorid);
    ...
    options.init_instance_callback = "M.editor_tinymce.init_callback";
...
M.editor_tinymce.init_callback = function(editor) {
    var editor = tinyMCE.editors[editor.id];
    if (document.readyState == "complete" && editor.getDoc().readyState === "complete") {
        console.log('done');
        M.util.js_complete("editor:" + editor.id);
    } else {
        console.log('waiting');
        setTimeout("M.editor_tinymce.init_callback({id : '" + editor.id + "'})", 100);
    }
}
