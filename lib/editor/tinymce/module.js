...
M.editor_tinymce.filepicker = function(target_id, url, type, win) {
    YUI().use('core_filepicker', function (Y) {
        ...
        // Calculate the zindex of the tinyMCE popup.
        var parentzindex = Y.one(parent.document.getElementById(win.name.slice(0, -4))).getStyle('zIndex');
        options.zIndex = parseInt(parentzindex) + 1;
