...
function xmldb_elluminate_upgrade($oldversion = 0) {
   ...
   if ( $oldversion < 2014122201 ) {
      ...
      $table = new xmldb_table('elluminate');
      $customcompletionstartfield = new xmldb_field('customcompletionstate');
      $customcompletionstartfield -> set_attributes(XMLDB_TYPE_INTEGER, '1', null, XMLDB_NOTNULL, null, '0','gradesession');
      if (!$dbman->field_exists($table, $customcompletionstartfield)) {
         $dbman->add_field($table, $customcompletionstartfield);
      }
      upgrade_mod_savepoint(true, 2014122201, 'elluminate');
