   ...
   foreach ($recordings as $recording) {
      try {
         if (!empty($recording->meetingid)) {
            ...
            //MOOD-620
            if (empty($full_recordings)){
               $logger->debug("Recording v3 Update: Invalid SAS Recording: " . $recording->recordingid);
               continue;
            }
         }else{
      ...
      foreach ($full_recordings as $full_recording) {
         if ($full_recording->recordingid == $recording->recordingid) {
            ...
            try {
               ...
               echo $OUTPUT->notification("Error during DB update for recording id [" .
                  $full_recording->recordingid . "], " . $errorDetails->getMessage());
            }
         }
      }
   }
