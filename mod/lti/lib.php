 ...
 * @param float $newmax
 */
function lti_rescale_activity_grades($course, $cm, $oldmin, $oldmax, $newmin, $newmax) {
    global $DB;
    ...
    $sql = 'UPDATE {lti_submission} SET originalgrade = (((originalgrade - :p1) * :p2) + :p3),
                                        gradepercent = (((gradepercent - :p4) * :p5) + :p6) WHERE ltiid = :lti';
    ...
    $lti = $DB->get_record('lti', $dbparams);
    $lti->cmidnumber = $cm->idnumber;

    lti_update_grades($lti);
 ...
 *
 * @param int $ltiid id of lti
 * @param int $userid optional user id, 0 means all users
 * @return array array of grades, false if none
 */
function lti_get_user_grades($lti, $userid = 0) {
    global $CFG, $DB;

    $params = array('ltiid1' => $lti->id, 'ltiid2' => $lti->id);
    $users = '';
    if ($userid != 0) {
        $users = ' AND u.id = :userid ';
        $params['userid'] = $userid;
    }
    $sql = "SELECT u.id, u.id AS userid, lti.gradepercent AS rawgrade
              FROM {user} u, {lti_submission} lti
              JOIN (
                  SELECT userid, MAX(datesubmitted) AS datesubmitted
                  FROM {lti_submission}
                  WHERE ltiid = :ltiid1
                  GROUP BY userid
              ) j1
              ON lti.datesubmitted = j1.datesubmitted AND lti.userid = j1.userid
              WHERE lti.ltiid = :ltiid2 AND u.id = lti.userid
              $users";
 ...
 */
function lti_update_grades($lti, $userid=0, $nullifnone=true) {
    global $CFG, $DB;
    require_once($CFG->libdir.'/gradelib.php');
    ...
    } else if ($grades = lti_get_user_grades($lti, $userid)) {
        lti_grade_item_update($lti, $grades);
