...
define('CELL_AUTO_WIDTH', -1);
define('CELL_FULLPAGE_WIDTH', 0);
 ...
 */
function certificate_print_text($pdf, $x, $y, $align, $font = 'freeserif', $style, $size = 10, $text, $width = 0, $maxwidth = 290) {
    $pdf->setFont($font, $style, $size);
    $x = $x == -1 ? '' : $x; // Set x to auto (margin) if -1.
    $pdf->SetXY($x, $y);
    if ($width == -1) {
        $width = 10 + $pdf->GetStringWidth($text);
        if ($width > $maxwidth) {
            $width = 0;
        }
    }
    $pdf->writeHTMLCell($width, 0, $x, $y, $text, 0, 0, 0, true, $align);
}

function certificate_get_xy($xycoordinates) {
    $textxy = explode(',', $xycoordinates);
    $textx = (int)trim(isset($textxy[0]) ? $textxy[0] : 10);
    if ($textx == -1) {
        $textwidth = CELL_FULLPAGE_WIDTH;
    } else {
        $textwidth = CELL_AUTO_WIDTH;
    }

    $texty = (int)trim(isset($textxy[1]) ? $textxy[1] : 195);

    return array($textx, $texty, $textwidth);
}
/**
 ...
 * @param int $h the height
 */
function certificate_print_background_image($cm, $pdf, $certificate, $x, $y, $w, $h) {
    $context = context_module::instance($cm->id);
    $fs = get_file_storage();
    $files = $fs->get_area_files($context->id, 'mod_certificate', 'certificatebackgroundimage', 0, 'itemid, filepath, filename', false);

    // Check that there is a file to process.
    if (count($files) == 1) {
        // Get the first (and only) file.
        $file = reset($files);
        $tempfilloc = $file->copy_content_to_temp(); // tcpdf will wirte stream to a temp file anyway.
        $pdf->Image($tempfilloc, $x, $y, $w, $h);
    }
