...
$pdf->SetMargins(30, 20, -1, true); // Reasonable margins selected according to border on a random certificate image.
...
certificate_print_background_image($cm, $pdf, $certificate, $brdrx, $brdry, $brdrw, $brdrh);
...
// certificate_print_text($pdf, $x, $y, 'C', $fontsans, '', 30, get_string('title', 'certificate')); // "Certificate of Achievement"
$pdf->SetTextColor(0, 0, 0);
// certificate_print_text($pdf, $x, $y + 20, 'C', $fontserif, '', 20, get_string('certify', 'certificate'));
list($namex, $namey, $namewidth) = certificate_get_xy($certificate->namexy);
certificate_print_text($pdf, $namex, $namey, 'C', $fontsans, '', 30, fullname($USER), $namewidth);
// certificate_print_text($pdf, $x, $y + 65, 'C', $fontsans, '', 20, get_string('statement', 'certificate'));
// certificate_print_text($pdf, $x, $y + 80, 'C', $fontsans, '', 20, format_string($course->fullname));

list($datex, $datey, $datewidth) = certificate_get_xy($certificate->datexy);
certificate_print_text($pdf, $datex, $datey, 'C', $fontsans, '', 14, certificate_get_date($certificate, $certrecord, $course),
                       $datewidth);
