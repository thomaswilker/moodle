...
function xmldb_certificate_upgrade($oldversion=0) {
    ...
    if ($oldversion < 2015083101) {
        $table = new xmldb_table('certificate');
        $field = new xmldb_field('datexy', XMLDB_TYPE_CHAR, '255', '-1,150', XMLDB_NOTNULL, 0, 0, 'datefmt');
        if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
        }
        $field = new xmldb_field('namexy', XMLDB_TYPE_CHAR, '255', '-1,95', XMLDB_NOTNULL, 0, 0, 'datexy');
        if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
        }
    }
