...
class assign {
     ...
     * @return bool
     */
    protected function submissions_open($userid = 0) {
        global $USER;
    ...
    protected function process_save_quick_grades() {
        ...
        foreach ($participants as $userid => $unused) {
            ...
            if ($modified >= 0) {
                ...
                $usergrade = required_param('quickgrade_' . $record->userid, PARAM_TEXT);
                $record->grade = unformat_float($usergrade);
        ...
        if ($gradinginstance) {
            ...
            if ($this->get_instance()->grade > 0) {
                ...
                if (count($grademenu) > 0) {
                    ...
                    $gradingelement = $mform->addElement('select', 'grade', $name, $grademenu);
